...
import {
  ...
  type SupplierPerformanceMetricKey as MetricKey,
  type SupplierPerformanceSelection,
  type SupplierPerformanceWeight as Weight,
} from "../lib/supplier-performance-lifecycle";
...
export default function SupplierPerformanceWorkspace({ toast }: { toast: Toast }) {
  ...
  const [loadedSelection, setLoadedSelection] = useState<SupplierPerformanceSelection | null>(null);
  ...
  const load = async (
    ...
    requestedTier = selectionRef.current.tier,
    signal = new AbortController().signal,
  ) => {
    const sequence = ++requestSequence.current;
    const result = await loadSupplierPerformanceSnapshot<Payload>({ quarter: requestedQuarter, tier: requestedTier, signal });
    const current = selectionRef.current;
    if (signal.aborted || sequence !== requestSequence.current || current.quarter !== requestedQuarter || current.tier !== requestedTier || result.kind === "stale") return;
    if (result.kind === "error") return toast(result.message);
    setData(result.snapshot.payload);
    setLoadedSelection({ quarter: requestedQuarter, tier: requestedTier });
    if (result.snapshot.weights) setWeights(result.snapshot.weights);
  };
  useEffect(() => {
    const controller = new AbortController();
    const selection = { quarter, tier };
    selectionRef.current = selection;
    const sequence = ++requestSequence.current;
    void loadSupplierPerformanceSnapshot<Payload>({ ...selection, signal: controller.signal })
      .then(result => {
        const current = selectionRef.current;
  ...
  const selectionIsCurrent = isSupplierPerformanceSelectionCurrent(loadedSelection, currentSelection);
  const currentData = selectionIsCurrent ? data : null;
  const weightsAreCurrent = selectionIsCurrent && weights.tier === tier;
  ...
  const saveWeights = async () => {
    let command;
    try {
      command = buildSupplierPerformanceWeightCommand(weights, loadedSelection, currentSelection, effectiveFrom);
    } catch (error) {
      return toast(error instanceof Error ? error.message : "当前层级权重仍在加载，请稍后重试");
    }
    if (Math.abs(total - 100) > 0.001) return toast("指标权重合计必须为 100%");
    setBusy(true);
    try {
      await writeSupplierPerformance(command);
