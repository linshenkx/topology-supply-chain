...
export default function MasterDataWorkspace({ toast }: { toast: Toast }) {
  ...
  const submitBom = () => {
    if (!bom.version.trim() || !bom.effectiveFrom) {
      toast("请先填写新版本号和生效日期");
      return null;
    }
    return { action: "create_bom", ...bom, components: bom.components.map(line => ({ ...line, quantityPerFinished: Number(line.quantityPerFinished), issueTolerance: Number(line.issueTolerance), consumptionTolerance: Number(line.consumptionTolerance), lossTolerance: Number(line.lossTolerance) })) };
  };
  ...
  return <section className="master-workspace">
    ...
    {tab === "sku" ? <>
      <div className="master-table sku-table"><div className="row header"><span>SKU 编码</span><span>名称</span><span>分类</span><span>库存单位</span><span>状态</span><span>操作</span></div>{data.skus.map(item => <div className={`row ${selectedSkuId === item.id ? "selected" : ""}`} key={item.id}><b>{item.code}</b><span>{item.name}</span><span>{itemTypeText[item.itemType ?? ""] || item.itemType}</span><span>{item.stockUnit}</span><span className={`state ${item.verificationStatus}`}>{item.verificationStatus === "approved" ? "已生效" : item.verificationStatus === "rejected" ? "已拒绝" : "待审批"}</span><span className="table-actions"><button type="button" onClick={() => setSelectedSkuId(item.id)}>查看详情</button>{item.verificationStatus === "rejected" && item.status === "draft" ? <button type="button" onClick={() => editRejectedSku(item)}>修改并重新提交</button> : null}</span></div>)}</div>
      {selectedSku && <div className="master-mask detail-mask"><div className="master-dialog detail-dialog"><div className="dialog-title"><div><h3>{selectedSku.code}</h3><p>{selectedSku.name}</p></div><button onClick={() => setSelectedSkuId(null)}>×</button></div><div className="detail-grid"><span>物料类型<b>{itemTypeText[selectedSku.itemType ?? ""] || selectedSku.itemType}</b></span><span>库存单位<b>{selectedSku.stockUnit || "未填写"}</b></span><span>状态<b>{selectedSku.status === "active" ? "生效" : selectedSku.status === "inactive" ? "停用" : "草稿"}</b></span><span>审批状态<b>{approvalText[selectedSku.verificationStatus] || selectedSku.verificationStatus}</b></span><span>生产超产容差<b>{bpsText(selectedSku.overproductionToleranceBps)}</b></span><span>采购超计划容差<b>{bpsText(selectedSku.purchaseOverToleranceBps)}</b></span><span>采购未足容差<b>{bpsText(selectedSku.purchaseUnderToleranceBps)}</b></span><span>最近审批<b>{selectedSku.latestApproval ? approvalText[selectedSku.latestApproval.status] || selectedSku.latestApproval.status : "无记录"}</b></span></div><div className="detail-subgrid"><div><h4>单位换算</h4>{selectedSkuConversions.length === 0 ? <p>无单位换算</p> : selectedSkuConversions.map(conversion => <p key={conversion.id}>{conversion.purchaseUnitQuantity} {conversion.purchaseUnit} = {conversion.stockUnitQuantity} {conversion.stockUnit}，{conversion.effectiveFrom} 生效，{conversion.status === "active" ? "启用" : "停用"}</p>)}</div><div><h4>驳回原因</h4><p>{selectedSku.latestApproval?.status === "rejected" ? selectedSku.latestApproval.reviewComment || "审批人未填写原因" : "无驳回原因"}</p></div></div><div className="dialog-actions"><button onClick={() => setSelectedSkuId(null)}>关闭</button>{selectedSku.verificationStatus === "rejected" && selectedSku.status === "draft" ? <button className="primary" onClick={() => editRejectedSku(selectedSku)}>修改并重新提交</button> : null}</div></div></div>}
    </> : <>
      ...
      {selectedBom && <div className="master-mask detail-mask"><div className="master-dialog detail-dialog"><div className="dialog-title"><div><h3>{selectedBom.finishedSku} / {selectedBom.version}</h3><p>{selectedBom.effectiveFrom} 至 {selectedBom.effectiveTo || "长期"}</p></div><button onClick={() => setSelectedBomId(null)}>×</button></div><p className="detail-rule">{selectedBom.overlapAllowed ? `允许与其他版本并行；原因：${selectedBom.overlapReason || "未填写"}` : "本版本生效后，系统按审批结果自动结束旧版本。"}</p><div className="bom-component-grid header"><span>物料</span><span>类型</span><span>单位用量</span><span>核心配件</span><span>领料/消耗/损耗偏差</span></div>{selectedBomComponents.map(component => <div className="bom-component-grid" key={component.id}><b>{component.componentSku}</b><span>{itemTypeText[component.itemType || ""] || "—"}</span><span>{component.quantityPerFinished}</span><span>{component.isCore ? "是" : "否"}</span><span>{bpsText(component.issueToleranceBps)} / {bpsText(component.consumptionToleranceBps)} / {bpsText(component.lossToleranceBps)}</span></div>)}<div className="dialog-actions"><button onClick={() => setSelectedBomId(null)}>关闭</button><button className="primary" onClick={() => copyBom(selectedBom)}>复制为新版本</button></div></div></div>}
    ...
    {open && <div className="master-mask"><div className="master-dialog"><div className="dialog-title"><h3>{open === "sku" ? skuEditingId === null ? "新增 SKU" : "修改并重新提交 SKU" : bomCopySourceId === null ? "新增 BOM 版本" : "复制 BOM 为新版本"}</h3><button onClick={closeDialog}>×</button></div>
      ...
      <div className="dialog-actions"><button onClick={closeDialog}>取消</button><button className="primary" disabled={busy} onClick={() => { const payload = open === "sku" ? submitSku() : submitBom(); if (payload) void submit(payload); }}>{busy ? "提交中…" : "提交审批"}</button></div>
    </div></div>}
  </section>;
}
