...
export default function AuditWorkspace({ toast }: { toast:(message:string)=>void }) {
  ...
  useEffect(()=>{const controller=new AbortController();void fetch(`/api/v1/audit-logs?${params(1)}`,{cache:"no-store",signal:controller.signal}).then(async response=>{const data=await response.json();if(!response.ok)throw new Error(data.error||"日志加载失败");return data;}).then(data=>{if(controller.signal.aborted)return;setLogs(data.logs||[]);setTotal(data.total||0);setPage(1);}).catch(error=>{if(!controller.signal.aborted)toast(error instanceof Error?error.message:"日志加载失败");}).finally(()=>{if(!controller.signal.aborted)setLoading(false);});return()=>controller.abort();},[]);
