...
export default function AuditWorkspace({ toast }: { toast:(message:string)=>void }) {
  ...
  const [logs,setLogs]=useState<LogRow[]>([]); const [page,setPage]=useState(1); const [total,setTotal]=useState(0); const [loading,setLoading]=useState(true);
  ...
  const initialRequestFailed = useEffectEvent((error: unknown) => toast(error instanceof Error ? error.message : "日志加载失败"));
  useEffect(()=>{const controller=new AbortController();void fetch("/api/v1/audit-logs?page=1&pageSize=20&archiveScope=active",{cache:"no-store",signal:controller.signal}).then(async response=>{const data=await response.json();if(!response.ok)throw new Error(data.error||"日志加载失败");return data;}).then(data=>{if(controller.signal.aborted)return;setLogs(data.logs||[]);setTotal(data.total||0);setPage(1);}).catch(error=>{if(!controller.signal.aborted)initialRequestFailed(error);}).finally(()=>{if(!controller.signal.aborted)setLoading(false);});return()=>controller.abort();},[]);
