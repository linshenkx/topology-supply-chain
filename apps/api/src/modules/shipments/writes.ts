...
import { requireShipmentRow } from "./support.js";
...
function requireFactoryBinding(command: OperationsCommandContext, factoryId: number): void {
...
export async function shipmentCommand(
  ...
  const current = await requireShipmentRow(command, id);
