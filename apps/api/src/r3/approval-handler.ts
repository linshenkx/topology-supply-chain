...
async function approvalBinding(
  ...
  transaction: R3CommandContext["transaction"],
  approval: Row,
  ...
  const r2Key = `r2.${workflow}`;
  if (registered.has(r2Key)) {
    ...
    return { effectKey: r2Key, effectAction: decision === "approved" ? "approve" : "reject",
  ...
  if (R3_WORKFLOWS.has(workflow) && registered.has(workflow)) {
...
async function approvalRow(transaction: R3CommandContext["transaction"], id: number): Promise<Row> {
...
async function effectApproval(transaction: R3CommandContext["transaction"], approvalId: number): Promise<Row> {
  const rows = await transaction.query<Row>(
...
export function registerR3ApprovalEffects(context: DomainRegistrationContext): void {
