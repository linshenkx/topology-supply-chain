  ...
  register(context) {
    context.approvalEffects.register({
      ...
      async execute({ transaction, claim }) {
        ...
        if (updated.affectedRows !== 1) throw new Error("Supply probe version CAS failed");
...
test("Operations MySQL commands preserve inventory, finance, approval, audit, outbox and replay invariants", {
  skip: !databaseUrl && "set MYSQL_OPERATIONS_TEST_URL to run Operations MySQL integration",
  timeout: 90_000,
}, async (t) => {
  ...
  const app = await buildRuntimeApp({
    logger: false,
    database: db,
    registrationManifests: [supplyProbeManifest, operationsManifest],
  ...
  const supplyApproval = await db.execute(
    `INSERT INTO approval_requests (
       request_no, workflow_type, entity_type, entity_id, summary, payload_json,
       high_risk, status, requested_by, requested_at, created_at, updated_at
  ...
  await db.execute(
    `INSERT INTO resource_versions (resource_type, resource_id, version, updated_at)
     VALUES ('approval_request', ?, 7, CURRENT_TIMESTAMP(3))`, [String(supplyApproval.insertId)],
  );
  const supplyDecision = { id:supplyApproval.insertId, decision:"approved", comment:"Supply reachable" };
  const supplyChallenge = await challenge(db, admin, {
    action:"approve", objectType:"r2:approval_request", objectId:supplyApproval.insertId,
    objectVersion:7, requestDigest:payloadDigest(supplyDecision),
  });
  const supplyResponse = await app.inject({ method:"POST", url:"/api/v1/approvals",
    headers:headers(admin, "approvals.decide", { ...supplyDecision, challengeNo:supplyChallenge }),
    payload:{ ...supplyDecision, challengeNo:supplyChallenge } });
  assert.equal(supplyResponse.statusCode, 200, supplyResponse.body);
  const [supplyVersion] = await db.query(
    `SELECT version FROM resource_versions WHERE resource_type = 'approval_request' AND resource_id = ?`,
    [String(supplyApproval.insertId)],
  );
  assert.equal(supplyVersion.version, 8);
