...
} from "../../../packages/contracts/dist/r3-fulfillment-writes.js";
import { buildRuntimeApp } from "../dist/runtime.js";
import r3Manifest from "../dist/r3/manifest.js";

const root = new URL("../../../", import.meta.url);

test("R3 exposes exactly the 13 delegated route-method commands", () => {
  assert.equal(Object.keys(R3_COMMANDS).length, 13);
  assert.deepEqual(new Set(Object.values(R3_COMMANDS)), new Set([
    "approvals.decide",
    "inventory.reserve",
  ...
  assert.deepEqual(Object.keys(R3_COMMAND_RESOURCES).sort(), Object.values(R3_COMMANDS).sort());
  assert.equal(r3Manifest.id, "r3.fulfillment-writes");
});

test("R3 manifest registers all route-method pairs without modifying runtime", async (t) => {
...
test("R3 stays inside Scope A boundaries", async () => {
  const sources = await Promise.all([
    "apps/api/src/r3/production-handlers.ts",
    "apps/api/src/r3/logistics-handlers.ts",
    "apps/api/src/r3/finance-handler.ts",
  ].map((path) => readFile(new URL(path, root), "utf8")));
...
test("bounded A-J guards are represented in contracts, locks, ACLs and UI", async () => {
  const [approval, finance, production, logistics, inventory, support, command, contract, client, page, shipping, sql] = await Promise.all([
  ...
  assert.match(inventory, /snapshotAvailable[\s\S]+snapshotLocked[\s\S]+snapshotDefective[\s\S]+snapshotPendingInspection/u);
  assert.match(inventory, /round1CountedBy/u);
  assert.match(support, /s\.scope = 'full_warehouse'/u);
  assert.match(support, /category, entity_type AS entityType, entity_id AS entityId/u);
...
test("production completion accepts zero so underproduction reaches the domain handler", async (t) => {
  const app = await buildRuntimeApp({ logger:false, registrationManifests:[r3Manifest], environment:{ NODE_ENV:"test" } });
  t.after(() => app.close());
