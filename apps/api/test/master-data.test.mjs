...
test("Next development bridge is GET-only and refuses production", () => {
  ...
  const evaluation = `
    ...
    const response = await route.GET(new Request(
      "http://127.0.0.1:3000/api/v1/master-data"
    ));
...
test("Next development bridge forwards only one valid session cookie to its fixed target", () => {
  ...
  const evaluation = `
    ...
    globalThis.fetch = async (url, options) => {
      ...
      return new Response(JSON.stringify({
        skus: [], conversions: [], boms: [], components: [], preview: true
      }), {
        status: 200,
        headers: {
          "content-type": "application/json",
          "set-cookie": "must-not-leak=1",
          "x-request-id": "proxy-development-check"
        }
      });
    };
    const route = await import(${JSON.stringify(routeUrl.href)});
    const response = await route.GET(new Request(
      "http://localhost:3000/api/v1/master-data?upstream=http://evil.test",
      {
        headers: {
          cookie: "theme=dark; topology_session=" + token + "; analytics=yes",
          "x-request-id": "proxy-development-check",
    ...
    await route.GET(new Request(
      "http://localhost:3000/api/v1/master-data",
  ...
  assert.deepEqual(body.captures[0], {
    url: "http://127.0.0.1:3001/api/v1/master-data",
    method: "GET",
    cookie: `topology_session=${"a".repeat(64)}`,
    requestId: "proxy-development-check",
    forgedIdentity: null,
  });
  assert.equal(body.captures[1].cookie, null);
  ...
  assert.equal(body.pragma, "no-cache");
  assert.equal(body.responseRequestId, "proxy-development-check");
