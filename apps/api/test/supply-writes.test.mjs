...
import { SUPPLY_COMMAND_BY_MUTATION, SUPPLY_COMMANDS } from "@topology/contracts/supply-writes";
...
import { executeSupplyCommand } from "../dist/platform/supply-command.js";
import manifest from "../dist/composition/supply-writes-manifest.js";
import { approvalNotification } from "../dist/modules/approvals/support.js";
import { requireFile } from "../dist/modules/files/support.js";
import { domainEvent } from "../dist/platform/supply-support.js";
...
function registrationContext(app, overrides = {}) {
  return {
    ...
    executeCommand: async () => { throw new Error("platform executor must not be used by supply"); },
    requireWriterFence: async () => {},
    authenticate: async () => ({ sessionId: 1, userId: 9, email: "r2@example.com", name: "Supply", roles: ["supply_chain"], factoryId: null, supplierId: null, organizationName: "Topology", localPreview: false }),
    authorize: () => false,
    audit: async () => {},
    enqueueOutbox: async () => {},
...
test("Supply manifest registers all 12 write mappings plus isolated approval effects", async (t) => {
  ...
  for (const mapping of Object.keys(SUPPLY_COMMAND_BY_MUTATION)) {
    const [method, path] = mapping.split(" ");
    assert.ok(paths[path]?.[method.toLowerCase()], mapping);
  }
  assert.equal(Object.keys(SUPPLY_COMMAND_BY_MUTATION).length, 12);
  assert.equal(SUPPLY_COMMANDS.length, 12);
...
test("Supply writes reject origin, CSRF, role-only, and mismatched factory bindings before UnitOfWork", async (t) => {
  let unitOfWorkCalls = 0;
  let access = { sessionId: 1, userId: 9, email: "r2@example.com", name: "Supply", roles: ["finance"], factoryId: null, supplierId: null, organizationName: "Topology", localPreview: false };
  const app = await buildApp({ logger: false });
...
test("each supply command route rejects missing Origin and CSRF before its domain handler", async (t) => {
  let unitOfWorkCalls = 0;
  const app = await buildApp({ logger: false });
  await manifest.register(registrationContext(app, {
    unitOfWork: async () => { unitOfWorkCalls += 1; throw new Error("must not run"); },
...
test("domain command executor fences, commits once, replays, and rejects digest reuse", async () => {
  ...
  const first = await executeSupplyCommand(options);
  const replay = await executeSupplyCommand(options);
  ...
  await assert.rejects(
    executeSupplyCommand({ ...options, payload: { action: "review", score: 4 } }),
    (error) => error.code === "IDEMPOTENCY_KEY_REUSED",
  );
});

test("Supply separates generic domain events from real approval notifications", async () => {
  const messages = [];
...
test("legacy writers are 410-only and frontend mutations use the v1 adapter/bridge", async () => {
  ...
  const imports = await readFile(new URL("apps/api/src/modules/imports/writes.ts", root), "utf8");
  const procurement = await readFile(new URL("apps/api/src/modules/purchase-plans/writes.ts", root), "utf8");
  const orders = await readFile(new URL("apps/api/src/modules/purchase-orders/writes.ts", root), "utf8");
  assert.match(imports, /entityType: "import_upload", entityIds: \[access\.userId\]/u);
  assert.equal((procurement.match(/entityType: "import_upload", entityIds: \[access\.userId\]/gu) ?? []).length, 1);
  assert.equal((orders.match(/entityType: "import_upload", entityIds: \[access\.userId\]/gu) ?? []).length, 1);
});
