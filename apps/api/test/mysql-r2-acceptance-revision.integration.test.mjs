...
const databaseUrl = process.env.MYSQL_R2_TEST_URL?.trim();
...
async function waitForOutbox(db, outboxId) {
  ...
  throw new Error("Worker did not consume the R2 outbox message");
}

async function runActualWorker(db, outboxId) {
  assert.equal(existsSync(workerEntry), true, "build @topology/worker before running the R2 acceptance integration");
...
test("R2 bounded acceptance revision counterexamples", {
  skip: !databaseUrl && "set MYSQL_R2_TEST_URL to run R2 acceptance integration",
  timeout: 120_000,
}, async (t) => {
  ...
  await manifest.register({
    ...
    executeCommand: async () => { throw new Error("R2 must not call platform executeCommand"); },
    requireWriterFence,
  ...
  await t.test("5/6 concurrent first-price approvals serialize on the supplier/SKU version", async () => {
    access = { ...access, userId: requesterId, roles: ["supply_chain"], factoryId: null };
    const price = (evidenceFileKey, effectiveFrom, amount) => ({ supplierId, sku: skuB, taxIncludedMinor: amount, taxExcludedMinor: amount - 100, taxRateBps: 1000, effectiveFrom, reason: "R2 acceptance", evidenceFileKey: String(evidenceFileKey) });
