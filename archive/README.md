...
- `source-snapshots/stage9-t2-legacy-routes-228de77b.tar` 是例外的 tracked source snapshot：它只保存 accepted T1 SHA 上 18 个已退役 route 的完整源码，受 `manifests/stage9-t2-legacy-source-snapshot.json` 与 `tooling/archive/verify-legacy-source-snapshot.mjs` 逐字恢复、敏感扫描和闭包验证约束。
- Manifest 只记录敏感扫描类别与次数，不记录匹配值；标为 `review` 的资产必须继续保持 ignored/protected，不能进入 Git 或 Docker context。
- 归档完成后运行 `node tooling/archive/archive-assets.mjs restore-dry-run > restore-plan.json`：命令只读核对每个 target 的 SHA-256、bytes、mtime，拒绝任何已存在的原路径或不可恢复父路径，并输出 `copy-exclusive-preserve-mtime` JSON 恢复计划；它不创建、移动、删除或覆盖资产。审核计划后可运行 `node tooling/archive/execute-archive-restore-plan.mjs restore-plan.json`；执行器在写入前重跑完整 preflight，只允许 exclusive copy，保留并复核 mtime/hash/bytes，永不移动、删除或覆盖已有路径。
