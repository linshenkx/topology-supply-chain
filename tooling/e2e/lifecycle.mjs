...
function transportForRun() { const value = argument("--transport") ?? "https"; if (value !== "http" && value !== "https") fail("--transport must be http or https"); return value; }
...
async function prepare() {
  ...
  const transport = transportForRun();
  ...
  const ports = { mysql: await reservePort(), stub: await reservePort(), worker: await reservePort(), api: await reservePort(), web: await reservePort(), gateway: await reservePort() };
  ...
  const browserOrigin = `${transport}://127.0.0.1:${ports.gateway}`;
  const state = { runId: id, createdAt: now(), transport, repositorySha: (await run("git", ["rev-parse", "HEAD"])).stdout.trim(), fixtureSha: await fixtureSha(), fixtureModuleSha: await fixtureModuleSha(), buildIdentity: await currentBuildIdentity(), fenceProfile: profile, databaseUrl, rootPassword, password: token(18), secrets: { signingKey: token(32), otpKey: token(32), stubKeys: { email: token(), sms: token(), scan: token() }, controlToken: token(), otpToken: token() }, resources: { container, dbName, ports, pids: {} }, origins: { browser: browserOrigin, ...(transport === "https" ? { https: browserOrigin } : { http: browserOrigin }), api: `http://127.0.0.1:${ports.api}`, worker: `http://127.0.0.1:${ports.worker}` } };
  try {
...
async function status() {
  ...
  checks.pids = Object.values(state.resources.pids).length === 5 && (await Promise.all(Object.values(state.resources.pids).map((record) => verifyOwnedProcess(state, record).catch(() => false)))) .every(Boolean) && (await Promise.all([state.resources.ports.stub, state.resources.ports.worker, state.resources.ports.api, state.resources.ports.web, state.resources.ports.gateway].map(portOpen))).every(Boolean);
