...
export async function seedScopeAFixture(connection, { runId, password }) {
  ...
  const sku = `${tag}-SKU`;
  await connection.execute("INSERT INTO skus (code,name,item_type,stock_unit,verification_status,status) VALUES (?,?,'finished_good','EA','approved','active')", [sku, `${tag} SKU`]);
  const accounts = {};
  const roleSpecs = [
  ...
  await connection.execute("INSERT INTO bom_components (bom_id,component_sku,item_type,is_core,quantity_per_finished) VALUES (?,?,'component',true,1)", [bomId, sku]);
  ...
  const approvalId = await insert(connection,
    "INSERT INTO approval_requests (request_no,workflow_type,entity_type,entity_id,summary,payload_json,high_risk,status,requested_by) VALUES (?, 'supplier_change','supplier',?,?,'{}',false,'pending',?)", [`${tag}-APR`, supplierId, `${tag} pending approval`, accounts.supply_chain]);
  ...
  const orderItemId = await insert(connection,
    "INSERT INTO order_items (purchase_order_id,sku,product_name,item_type,supplier_id,quantity,unit_price_tax_included_minor,amount_tax_included_minor,due_date) VALUES (?,?,?,'finished_good',?,10,100,1000,'2026-02-01')", [purchaseOrderId, sku, `${tag} SKU`, supplierId]);
  ...
  return { accounts, entities: { factoryId, supplierId, sku, bomId, warehouseId, batchId, supplierSkuId, approvalId, planId, planItemId, purchaseOrderId, orderItemId, executionOrderId, qualityInspectionId, stocktakeId, shipmentId, returnId, invoiceId, paymentRequestId } };
}
