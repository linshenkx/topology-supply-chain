...
if [[ $# -lt 2 ]]; then
  echo "用法：tooling/release/activate-writers.sh <activation-evidence.json> <resource> [resource ...]"
...
DEPLOY_DIR="${REPOSITORY_ROOT}/infrastructure/aliyun"
cd "${DEPLOY_DIR}"
if [[ ! -f ".env.production" ]]; then
  echo "缺少 infrastructure/aliyun/.env.production，停止激活。"
  exit 1
fi
...
docker run --rm "topology-scm-migrator:${RELEASE_TAG}" node tooling/release/release-manifest.mjs print > "${CURRENT_MANIFEST}"
...
docker compose --profile migration run --rm \
  ...
  migrator node tooling/release/set-writer-fences.mjs /tmp/writer-activation-evidence.json
