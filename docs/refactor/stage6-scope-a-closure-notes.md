...
## 2026-08-12 — Wave 3 command executor core closure

- Exact implementation baseline: `6fa2924dc8f94f6ea2ecf63c1e6351564822b89b` in an isolated, initially clean detached worktree. The platform, R2, and R3 idempotent executor state machines now share one `executeIdempotentCommand` core; the R2 and R3 files are 41 and 48 non-empty lines respectively, while the three production files total 397 versus the 597-line baseline.
- Canonical `{command,payload}` digest bytes, actor scope, `(command, actor, key)` claim identity, writer-fence ordering and resource identity, pending/unknown/completed replay behavior, guarded result persistence, transaction rollback, and unknown-commit sanitization are unchanged. R2 result-dependent 2xx validation and missing-key wording, and R3 same-origin/CSRF/authentication plus non-string digest-header compatibility, remain explicit adapter policy.
- Table-driven adapter parity checks bind all 35 command identities to the unchanged release protocol, whose manifest still contains 29 resources. A disposable MySQL 8.4 run verified concurrent single execution and replay, digest mismatch rejection, successful-commit/unknown-response recovery, disabled and owner-mismatched fences, and business-error rollback of command, domain, audit, and outbox rows; its task-named container was removed after verification.
