...
| S12-B | 待检批次 → 整批质检 → 放行/隔离 | 整批 pass/fail；pending 转 available 或 quarantine；quality_inspections + audit + InspectionCompleted/DispositionRequired 可取证；部分/抽样/混合/来源歧义负路径符合预期 |
| S12-C | 生产预留 → 领料/消耗 → 释放 → 完工 | inventory_reservations/production_material_lines/inventory_batches 守恒；release 零预留 409 零副作用；audit + 对应 Outbox 可取证；下降/超额/冻结/越权负路径符合预期 |
